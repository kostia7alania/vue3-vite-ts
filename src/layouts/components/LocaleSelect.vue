<template>
  <VButton class="tw-relative tw-uppercase" @click="isOpen = !isOpen">
    <div class="tw-flex tw-items-center tw-gap-x-2 tw-text-black-1 tw-text-sm">
      <div>{{ $i18n.locale }}</div>
      <div>
        <svg
          width="11"
    ...
    <div
      class
      :class="{ ' tw-absolute tw-top-6': isOpen, 'tw-hidden': !isOpen }"
      @click="localeToggle"
    >{{ $i18n.locale === 'en' ? 'LT' : 'EN' }}</div>
  </VButton>
</template>
