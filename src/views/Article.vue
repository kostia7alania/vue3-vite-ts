<template>
  <div>
    ...
    <section
      class="max-layout-width-overflow-auto tw-pt-16 tw-pb-20 xl:tw-pb-30 tw-flex tw-flex-col tw-gap-y-4"
    >
      <div class="px-16px-48px-80px">
        <!--  -->
        <div v-if="article?.content?.length" class="tw-flex tw-flex-col tw-gap-y-4">
          <div v-for="(item, i) of article.content" :key="i" class="tw-flex">
            ...
            <div
              class="tw-flex-1 tw-mb-4 tw-text-sm tw-leading-6 content-container"
              :class="{
                'tw-max-w-3xl': item.type !== 'image_fullscreen',
              }"
            >
              <div v-if="item.type === 'text_and_title'">
                <div class="tw-font-semibold tw-mb-6 tw-text-xl">{{ item.content.title }}</div>
                <Markdown :source="item.content.text" />
              </div>
              <Markdown v-else-if="item.type === 'text_only'" :source="item.content.text" />
